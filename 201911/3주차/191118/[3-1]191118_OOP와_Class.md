...
- 값은 `==`, 인스턴스는 `===` 연산자로 비교한다
- Class에서 값은 heap에 있는 인스턴스의 주소값이다
- Class에서 객체 간에 내부 값에 대한 비교는 heap에 있는 실제 값에 대한 비교이므로 Equatable 프로토콜을 구현해서 `==` 연산자를 구현하고, 비교할 값을 설정해야한다
- Class에서 객체 자체에 대한 비교는 곧 각 객체가 가진 인스턴스의 주소값에 대한 비교이고, `===` 연산자를 이용해서 비교한다
- Class 안에서 `self` 키워드는 class 내에서 해당 class의 인스턴스의 메모리 주소를 가짐. `self`를 이용한 접근은 인스턴스에 바로 접근하는 것
